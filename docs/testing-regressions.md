...
| Provider continuations must preserve visible content order, reject a kind run split across another kind, and retain OpenAI native function identity | `openAIContinuationRejectsVisibleContentReordering`, `deepSeekContinuationRejectsVisibleContentReordering`, `visibleContentCannotSplitANativeKindRunAcrossAnotherKind`, `openAIToolOnlyContinuationPreservesNativeIdentityAcrossEncoding` | `Tests/AgentProvidersTests/ResponsesContinuationIntegrityTests.swift` |
