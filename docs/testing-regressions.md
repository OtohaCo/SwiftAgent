...
| DeepSeek documented empty reasoning summaries and null encrypted metadata must remain replayable, while non-null encrypted state stays rejected | `deepSeekDocumentedReasoningMetadataRemainsReplayable`, `deepSeekNonNullEncryptedReasoningMetadataFailsClosed` | `Tests/AgentProvidersTests/ResponsesContinuationIntegrityTests.swift` |
