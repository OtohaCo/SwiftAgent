...
rc.1 的固定提交为 `d2347f11c6a78f421708e897dae42a51a98d37ea`。
**不要安装 rc.1，却直接使用 RC.2 文档介绍的全部 API。** OpenAI/DeepSeek Responses、Decision/Jev 等下一 RC 新增能力，需要对应的未发布 revision。请查看该 revision 的 Package 和指南。
...
这是明确的开发版本固定方式，不表示推荐发布未经审计的版本，也不表示它是最新 commit。只添加目标实际需要的 products。RC.2 开发分支是 `plan/swift-agent-rc2`；不要假设 `main` 已包含这些修改。参见[版本策略](docs/guides/swift-agent-versioning.md)。
